          ...
          <div key={task.id} className={`group bg-white dark:bg-slate-800 border rounded-xl transition-colors ${isEditing ? 'border-xero-green px-4 py-3' : 'border-gray-100 dark:border-slate-700 px-4 py-3 hover:border-gray-200'}`}>
            {isEditing ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0 py-2 cursor-pointer" onClick={() => toggleTask(task.id)}>
                  ...
                  <Linkified text={task.text} className={`block text-sm mt-0.5 ${task.done ? 'line-through text-gray-400' : 'text-gray-800 dark:text-slate-100'}`} />
                  {task.description && (
                    <p className="text-xs text-gray-400 dark:text-slate-500 mt-0.5 line-clamp-1"><Linkified text={task.description} /></p>
                  )}
                </div>
