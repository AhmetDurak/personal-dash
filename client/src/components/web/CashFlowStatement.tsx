      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-xero-bg text-xs uppercase tracking-wide text-gray-500 border-b border-xero-border">
            ...
            <th className="text-right px-4 py-2.5 font-medium">Cash Out</th>
        ...
        <tbody className="divide-y divide-xero-border">
          <tr className="bg-xero-bg/50">
            <td className="px-6 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide" colSpan={4}>Income</td>
          </tr>
          <tr className="hover:bg-gray-50 transition-colors">
            ...
            <td className="px-4 py-3 text-right text-xero-green font-semibold">{formatEur(summary.income)}</td>
            <td className="px-4 py-3 text-right text-gray-400">—</td>
            <td className="px-6 py-3 text-right text-xero-green font-semibold">{formatEur(summary.income)}</td>
          </tr>
          <tr className="bg-xero-bg/50">
            <td className="px-6 py-2.5 text-xs font-semibold text-gray-500 uppercase tracking-wide" colSpan={4}>Expenses</td>
          </tr>
          {EXPENSE_CATS.map(cat => {
            ...
            return (
              <tr key={cat} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-right text-gray-400">—</td>
                <td className="px-4 py-3 text-right text-gray-700">{formatEur(amount)}</td>
        ...
        <tfoot className="border-t-2 border-xero-border">
          <tr className="bg-xero-bg">
            <td className="px-6 py-3 font-semibold text-gray-800">Opening Balance</td>
            <td className="px-4 py-3" />
            <td className="px-4 py-3" />
            <td className="px-6 py-3 text-right font-semibold text-gray-700">{formatEur(prevEndBalance)}</td>
          </tr>
          <tr className="bg-xero-bg">
            <td className="px-6 py-3 font-bold text-gray-800">Net</td>
            <td className="px-4 py-3 text-right font-bold text-xero-green">{formatEur(summary.income)}</td>
            <td className="px-4 py-3 text-right font-bold text-red-500">{formatEur(summary.totalExpenses)}</td>
            <td className={`px-6 py-3 text-right font-bold text-base ${summary.net >= 0 ? 'text-xero-green' : 'text-red-500'}`}>{formatEur(summary.net)}</td>
          </tr>
          <tr className="bg-xero-bg">
            <td className="px-6 py-3 font-bold text-gray-900">Closing Balance</td>
            <td className="px-4 py-3" />
            <td className="px-4 py-3" />
            <td className={`px-6 py-3 text-right font-bold text-base ${closingBalance >= 0 ? 'text-xero-green' : 'text-red-500'}`}>{formatEur(closingBalance)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
