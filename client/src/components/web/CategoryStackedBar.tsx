...
interface Props { data: StackedDataset; title?: string }

export function CategoryStackedBar({ data, title = 'Expenses by Category' }: Props) {
      ...
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={chartData} barSize={20}>
          ...
          {data.categories.map(cat => (
            <Bar key={cat} dataKey={cat} stackId="a" fill={CAT_COLORS[cat as keyof typeof CAT_COLORS] ?? '#888'} />
          ))}
        </BarChart>
      </ResponsiveContainer>
    </div>
